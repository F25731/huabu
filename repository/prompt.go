...
func ListPrompts(q model.Query) ([]model.Prompt, int64, error) {
	...
	for i := range items {
		items[i].Category = normalizePromptCategory(items[i].Category)
		category, _ := PromptCategoryByCode(items[i].Category)
		items[i].GithubURL = category.GithubURL
	}
...
func applyPromptFilters(tx *gorm.DB, q model.Query) *gorm.DB {
	tx = tx.Where("category IN ?", promptCategoryAliases())
	if q.Keyword != "" {
		like := "%" + q.Keyword + "%"
		tx = tx.Where("title LIKE ? OR prompt LIKE ?", like, like)
	}
	if isActivePromptOption(q.Category) && !isPromptImage2Category(q.Category) {
...
func promptCategoryAliases() []string {
	return []string{"image-2", "gpt-image-2-prompts", "youmind-gpt-image-2", "davidwu-gpt-image2-prompts"}
}

func normalizePromptCategory(category string) string {
	if strings.Contains(category, "image-2") || strings.Contains(category, "image2") {
		return "gpt-image-2-prompts"
	}
	return category
}

func isPromptImage2Category(category string) bool {
	return normalizePromptCategory(category) == "gpt-image-2-prompts"
}
