...
- 知梦 API Key 余额会在前端登录态恢复后按 `NEXT_PUBLIC_TOKEN_USAGE_AUTO_REFRESH_INTERVAL_MS` 自动刷新，默认每 10 分钟一次；多个 Key 的余额请求会按 `NEXT_PUBLIC_TOKEN_USAGE_REQUEST_DELAY_MS` 间隔发送，默认间隔 3 秒。
- 画布右上角余额入口固定显示“用户余额”和状态点，不再直接显示单个 Key 余额；悬浮时展示 1k、2k、4k 三档余额进度条和 `$x / $y`，所有已配置 Key 余额均不少于一半时状态点显示绿色。
